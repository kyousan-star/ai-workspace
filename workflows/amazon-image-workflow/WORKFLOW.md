...
目的：把本项目锁定的产品形态、抠图、过审构图沉淀进 `visual-lab/asset-library/{product-line}/`，而不是只留在项目 `10_LEARNINGS/` 里——下次同产品线新SKU/新场景直接复用，不用重新提取。
...
- 若 `asset-library/{product-line}/` 已存在，检查本项目产品形态是否与已锁定描述一致：一致则直接复用不重复提取；有差异则更新并标注差异原因。
- 若不存在，新建（复制 `asset-library/_template/`）。
- 环境没有内部抠图API，本阶段只产出抠图任务清单交给外部工具/人工处理，不假装能自动完成。

### 输出

- `visual-lab/asset-library/{product-line}/product-facts-locked.md`（新建或更新）
- `visual-lab/asset-library/{product-line}/cutouts/` 任务清单（待处理）
- `visual-lab/asset-library/{product-line}/brand-style.md`（如有品牌资料）
...
项目内`10_LEARNINGS/`只对本项目可见，不回流则下次同产品线项目用不上。本阶段必须把可复用部分写回：

- 实际上线/过审的构图 → `visual-lab/asset-library/{product-line}/approved-compositions.md`
- 成功与失败的prompt → `visual-lab/asset-library/{product-line}/prompt-library.md`（失败案例同样要写，防止重复踩坑）

只写项目内文件、不回写资产库，视为本阶段未完成。
