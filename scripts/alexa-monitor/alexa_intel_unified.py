...
def fetch_sorftime_keyword(kw_cfg: dict) -> dict:
    ...
    # Sorftime 2026-07 接口改版：参数改下划线（keyword_support_site/position_type），
    # 返回体包一层 {"doc":..., "data":...}，字段改英文。旧驼峰参数会被忽略并返回
    # "Please specify the site" 错误文本 → 需取 .data 并容错。
    detail_resp = _call_sorftime("keyword_detail",
    ...
    search_resp = _call_sorftime("keyword_search_results",
                                 {"keyword": keyword, "keyword_support_site": "US",
                                  "page": 1, "position_type": 1})
    page1 = search_resp.get("data") if isinstance(search_resp, dict) else None
    ...
    return {
        "keyword":        keyword,
        "group":          kw_cfg["group"],
        "weekly_search":  _parse_int(detail.get("weekly_search_volume", 0)),
        "monthly_search": _parse_int(detail.get("monthly_search_volume", 0)),
        "cpc":            _parse_float(detail.get("recommended_cpc_bid", 0)),
        "competition":    str(detail.get("search_result_competitor_count", "")),
    ...
    try:
        ...
        listing_text = page.evaluate("""() => {
            ...
            return {title: t('#productTitle'), bullets: bullets.slice(0, 8),
                    ...
                    description: t('#productDescription').slice(0, 1500),
                    aplus: t('#aplus, #aplus_feature_div, #dpx-aplus-product-description_feature_div').slice(0, 2500)};
        }""")
...
def build_insight_facts(current: dict, wow: dict, prev: dict | None) -> dict:
    ...
    for asin, item in current.get("own_qa", {}).items():
        ...
        own_text = " ".join([lt.get("title", ""), " ".join(lt.get("bullets", []) or []),
                             lt.get("box", ""), lt.get("description", ""), lt.get("aplus", "")])
        own_qa_facts[asin] = {
            # 完整前台文案喂给 Claude，供其核对"某信息是否已写"，避免建议补已有内容
            "listing_title": lt.get("title", "")[:200],
            "listing_bullets": [str(b)[:300] for b in (lt.get("bullets", []) or [])][:8],
            "listing_description": (lt.get("description", "") or "")[:1200],
            "listing_aplus_text": (lt.get("aplus", "") or "")[:1500],
            "listing_box": (lt.get("box", "") or "")[:300],
...
- 【防假阳性·必须严格执行】own_products 每个 ASIN 含 listing_title / listing_bullets / listing_description / listing_aplus_text / listing_box，即本品前台文案全文。提任何"补写/新增到 标题|五点|A+|description"的 action 前，**必须先在这几个字段里逐字检索该信息是否已存在**；已明确写到的（如夹持宽度、Android 兼容、是否随附收纳袋等），**禁止再建议"补"**，否则就是错误建议。
- 若某信息在 listing 文本里已写、但对应 QA 仍为 B/C/D：判为「Alexa 未准确读取该信息」，action 要写成"listing 已写 X（引用原文），但 Alexa 未准确应答 → 把该卖点做成结构化后台属性字段让 Alexa 可读 / 或在 QA 用更明确措辞复述"，target 用「后台属性」或「QA」；**绝不能表述成"listing 未写 X / 去标题五点补 X"**。
- 后台属性字段（Seller Central 结构化属性）与前台 description 中未被抓到的部分不在事实包内，不可断言其"未填/没写"；只有当 listing_title/bullets/description/aplus 里**也确实找不到**该信息、且 QA 显示 Alexa 答错或答不出时，才可建议补该信息，并注明落点。
