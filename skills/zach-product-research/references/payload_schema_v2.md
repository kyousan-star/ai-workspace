...
```json
{
  "decision_funnel": [
    {
      "stage": "start",
      "title": "起点：一个方向假设，不是一个产品",
      "desc": "输入方向「XXX」。已知锚点是什么，本轮要验证的假设是什么。",
      "data_points": ["锚点：来自哪个既有结论", "假设：具体在验证什么"],
      "why": "为什么从这个假设问起（给冷读者的定位）"
    },
    {
      "stage": "turn",
      "title": "关键转折：假设被证伪或需要人判断",
      "desc": "数据显示了什么，为什么原假设站不住，或出现了需要人拍板的分岔点。",
      "data_points": ["具体数字1", "具体数字2"],
      "why": "这一步筛掉/推进了什么判断"
    },
    {
      "stage": "gate",
      "title": "验证关卡：把「值不值得」量化",
      "desc": "过了哪些硬性快筛/建模，量化出什么结构性结论。",
      "data_points": ["Go/No-Go 分数", "关键量化结论"],
      "why": "为什么这步的结论是诚实的（不虚高/不凑数）"
    },
    {
      "stage": "final",
      "title": "落地：一个具体结论，不是一个方向",
      "desc": "收敛到的最终具体建议（规格/定价/行动）。",
      "data_points": ["最终规格或数字"],
      "why": "这条链路如何回答「why this」"
    }
  ],
  "conclusions": [
    {
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `decision_funnel` | array | ⛔ | **≥4 步**，是「为什么是这个结论」的推导链，写给第一次读报告、不了解调研过程的人。第一步 `stage` 必须是 `start`，最后一步必须是 `final`，中间用 `turn`（转折/需要人判断的分岔点）和 `gate`（量化验证关卡）标记。每步含 `stage`/`title`/`desc`/`data_points`/`why`，`why` 回答"这一步为什么重要/筛掉了什么"。**这不是选品方法论的品牌名，是本报告自己的推导过程——不要在报告正文里用任何第三方工具/人名代称这个阶段**（如"XX深挖"），改用描述性说法如"定向验证阶段""建模量化阶段" |
...
3b. `ch01_executive_summary.decision_funnel` 长度 ≥ 4，每步字段完整，首步 stage=start，末步 stage=final（HTML 中还会检查是否渲染出"需求漏斗"区块）
