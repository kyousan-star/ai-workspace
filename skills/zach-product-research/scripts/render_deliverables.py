...
def _render_ch01_md(ch: dict[str, Any]) -> str:
    ...
    funnel = ch.get("decision_funnel", [])
    if funnel:
        lines.append("### 需求漏斗：为什么是这个结论（写给第一次读者）\n")
        lines.append(
            "> 跳过这段直接看结论，容易误以为这是拍脑袋判断；完整看完，"
            "每一步被证伪/证实的判断和留下的理由都在这里。\n"
        )
        for i, step in enumerate(funnel, 1):
            stage_label = _FUNNEL_STAGE_LABEL.get(step.get("stage", ""), "")
            title = _safe(step.get("title"))
            desc = _safe(step.get("desc"))
            data_points = step.get("data_points", [])
            why = _safe(step.get("why"))
            tag = f"[{stage_label}] " if stage_label else ""
            lines.append(f"{i}. **{tag}{title}** — {desc}")
            if data_points:
                lines.append("   - 数据：" + "；".join(_safe(d) for d in data_points))
            if why:
                lines.append(f"   - ↳ {why}")
        lines.append("")
...
def render_html_report(chapters: dict[str, Any], metadata: dict[str, Any]) -> str:
    ...
    # -- Decision funnel (why this: 需求来源链，写给第一次读者) --
    funnel_html = ""
    for i, step in enumerate(ch01.get("decision_funnel", []), 1):
        stage = _h(step.get("stage", ""))
        stage_label = _FUNNEL_STAGE_LABEL.get(step.get("stage", ""), "")
        title = _h(step.get("title"))
        desc = step.get("desc", "")  # 允许内联 <b> 标记，见 schema 说明
        data_tags = "".join(
            f'<span class="funnel-data">{_h(d)}</span>' for d in step.get("data_points", [])
        )
        why = _h(step.get("why"))
        funnel_html += (
            f'<div class="funnel-step funnel-{stage}">'
            f'<div class="funnel-num">{i}</div>'
            f'<div class="funnel-body">'
            f'<div class="funnel-title">{f"[{stage_label}] " if stage_label else ""}{title}</div>'
            f'<div class="funnel-desc">{desc}</div>'
            f'<div>{data_tags}</div>'
            f'{f"<div class=funnel-why>↳ {why}</div>" if why else ""}'
            f'</div></div>\n'
        )
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .funnel-box {{ background: linear-gradient(180deg,var(--gray-50) 0%,#fff 100%);
      border: 1px solid var(--gray-200); border-radius: 14px; padding: 22px 20px 6px; }}
    .funnel-note {{ font-size: 13px; color: var(--gray-500); margin-bottom: 16px; }}
    .funnel-step {{ display: flex; gap: 14px; position: relative; padding-bottom: 24px; }}
    .funnel-step:not(:last-child)::before {{ content: ""; position: absolute; left: 17px; top: 36px;
      bottom: -6px; width: 2px; background: var(--gray-200); }}
    .funnel-num {{ flex: 0 0 36px; height: 36px; border-radius: 50%; background: #fff;
      border: 2px solid var(--blue); color: var(--blue); display: flex; align-items: center;
      justify-content: center; font-weight: 800; font-size: 14px; z-index: 1; }}
    .funnel-turn .funnel-num {{ border-color: var(--orange); color: var(--orange); background: #fff7ed; }}
    .funnel-gate .funnel-num {{ border-color: var(--red); color: var(--red); background: #fef2f2; }}
    .funnel-final .funnel-num {{ border-color: var(--green); color: var(--green); background: #f0fdf4; }}
    .funnel-body {{ flex: 1; padding-top: 1px; }}
    .funnel-title {{ font-weight: 700; font-size: 14.5px; color: var(--gray-900); }}
    .funnel-desc {{ color: var(--gray-700); font-size: 13px; margin: 4px 0 6px; }}
    .funnel-data {{ display: inline-block; background: var(--gray-100); border: 1px solid var(--gray-200);
      border-radius: 6px; padding: 2px 8px; font-size: 12px; color: var(--gray-700);
      margin: 2px 6px 2px 0; font-family: ui-monospace, monospace; }}
    .funnel-why {{ font-size: 12px; color: var(--gray-500); font-style: italic; margin-top: 4px; }}
  ...
  <div class="container">
    ...
    {f'''<section>
      ...
      <div class="funnel-box">
        <div class="funnel-note">写给第一次看报告的人——跳步骤直接看结论，容易误以为是拍脑袋判断；完整看完，每一步被证伪/证实的判断都在这里。</div>
        {funnel_html}
      </div>
    </section>''' if funnel_html else ''}
...
def _build_dashboard_view_model(payload: dict[str, Any], chapters: dict[str, Any], metadata: dict[str, Any]) -> dict[str, Any]:
    ...
    for o in ch07.get("opportunities", []):
        gaps.append({
            ...
            "产品数": o.get("existing_products", ""),
            "月销量": o.get("existing_sales", ""),
            "action": o.get("action", ""),
        })
    ...
    for t in ch08.get("tiers", []):
        ...
        recommendations.append({
            "tier": t.get("tier", ""),
            "title": (t.get("name", "") + "：" + t.get("differentiation", "")).strip("："),
            "body": ("对标 " + t.get("benchmark_asin", "") + " —— " + t.get("benchmark_advantage", "")) if t.get("benchmark_asin") else t.get("decision_rationale", ""),
            "price": t.get("target_price", ""),
            "points": [f"{k}：{v}" for k, v in specs.items()],
        })
    return {
        "hero": hero,
        "rings": rings,
        ...
        "dataAudit": payload.get("excel_sheets", {}).get("数据溯源", []),
        "recommendations": recommendations,
        "discoveryPath": payload.get("discovery_path", {}),
...
def validate_html(html_path: Path) -> list[str]:
    ...
    if "需求漏斗" not in content:
        errors.append('HTML 缺少"需求漏斗：为什么是这个结论"区块（ch01.decision_funnel 未渲染，检查 payload 是否提供）')
    return errors
...
def validate_v2_chapters(chapters: dict[str, Any]) -> list[str]:
    ...
    funnel = ch01.get("decision_funnel", [])
    if len(funnel) < 4:
        errors.append(
            f"ch01 缺少 decision_funnel 或步骤不足: {len(funnel)}（需 ≥ 4）——"
            "这是「为什么是这个结论」的推导链，写给第一次读报告的人，不是可省略的装饰。"
            "见 references/payload_schema_v2.md 的 decision_funnel 说明"
        )
    else:
        valid_stages = {"start", "turn", "gate", "final"}
        for i, step in enumerate(funnel, 1):
            missing = [f for f in ("stage", "title", "desc", "why") if not step.get(f)]
            if missing:
                errors.append(f"ch01.decision_funnel 第{i}步缺少字段: {', '.join(missing)}")
            if step.get("stage") not in valid_stages:
                errors.append(f"ch01.decision_funnel 第{i}步 stage 非法: {step.get('stage')}（须为 start/turn/gate/final）")
        if funnel[0].get("stage") != "start":
            errors.append("ch01.decision_funnel 第1步 stage 必须是 start（起点假设）")
        if funnel[-1].get("stage") != "final":
            errors.append("ch01.decision_funnel 最后一步 stage 必须是 final（落地结论）")
