...
10b. ⛔ **必须有需求漏斗（decision_funnel），写给第一次读报告的人**：`ch01.decision_funnel` ≥4 步，从最初的方向/假设输入开始，一路标出被证伪的判断（`turn`）、量化验证关卡（`gate`），收敛到最终具体结论（`final`）。这条规则的动机：只给"CONDITIONAL GO 6.2"这类结论，冷读者看不到为什么不是别的答案，会反问"why this"——**不要等用户提醒才补，每次组装 ch01 时默认必须构建这条链**。渲染器（`render_deliverables.py`）会硬校验：缺失或步骤不足 4 步直接 validate 失败。写法：见 `references/payload_schema_v2.md` 的 `decision_funnel` 字段说明。**这条链路属于本次分析自己的推导过程，报告正文里不要用任何第三方工具名/人名指代某个阶段**（例如不要写"XX深挖阶段"），改用描述性说法（"定向验证阶段""建模量化阶段""可行性快筛阶段"）。
...
## 弱模型执行协议（执行者非 Fable 时强制）

执行者为 Opus/Sonnet/Haiku 时，在上述硬性规则之外追加：

1. ⛔ **结论权分离**：GO / CONDITIONAL GO 不得作为最终结论直接输出，只能输出「候选判定 + 完整证据链」，结论处标注 `【候选结论 · 待终审】`，由 Fable 或用户终审后定稿；No-Go / Hold 可直接给出（保守方向错误代价低）。
2. ⛔ **变体铁律**（实测项目翻盘教训）：每个对标/标杆 ASIN 必须跑 `product_variations` + 核对上架时间。销量口径用子体真实值，评论壁垒口径用买家页面所见（家族合并）。"低评论撑高销"样本先默认是变体家族错觉，举证后才能采信。
3. ⛔ **同比先于环比**：任何"暴跌/暴涨"结论必须对照去年同月（YoY）；只有环比的趋势判断不得进结论。
4. ⛔ **validate 是硬闸**：交付前必须跑 `render_deliverables.py validate` 且通过；失败时修 payload，禁止绕过渲染器手工补文件。
5. ⛔ **分数单一出处 + 机械重算**：全文只允许一个综合分数值；补充验证改变事实后必须机械重算评分，禁止"风险挂起不入分"。
6. ⛔ **每步留痕**：每个 Step 结束输出一行中间结论（数据口径 + 关键数字 + 该步判断），跑歪时可定位到步。
7. ⛔ **交付前反证**：结论输出前回答两问并写入报告——「这个结论最可能错在哪」「什么数据能推翻它」，并对答案实际做核查（至少 1-2 次 Sorftime 调用），不许纸面反证。
...
- [ ] **有需求漏斗**（`ch01.decision_funnel` ≥4步，首步start末步final，覆盖起点假设→转折/判断→验证关卡→落地结论，且未用第三方工具/人名代称阶段）？
