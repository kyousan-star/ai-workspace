...
## 面板终验下载清单（zach 出 GO/CONDITIONAL GO 且投入较大时）

交接报告末尾必须附一张「面板下载清单」，供用户从卖家精灵导出 24 个月数据、跑 `amazon-category-analysis` 终验：

| 项 | 内容 | 来源 |
|---|---|---|
| 下载节点 | 对标品散布的**所有**节点（P2/P3 已查到；缝隙形态常跨节点，只下主节点会漏） | P2 首页普查 + product_detail 节点字段 |
| 目标形态定义 | 一句话描述我方形态 + 排除形态（供打标"目标形态相关性"维度用，非目标形态不得混入统计） | P3 形态速览 |
| 待证实/证伪清单 | blue-ocean+zach 的关键结论各一行（如"竞争浅：首页<100评占比X%"、"疑隐性垄断：品牌A/B/C 同指纹"），终验逐条给证实/推翻 | 本次交接数据 |
| 价位段划分 | 按缝隙价格带设计，保证目标价格带单独成段 | P2 价格带证据 |

终验产出里最有增量价值的四项（Sorftime 实时快照拿不到）：新品死亡率、店铺 CR3/隐性垄断确认、均值/中位数市场结构、排名分层起盘锚点。
