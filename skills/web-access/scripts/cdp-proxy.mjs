...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // POST /type?target=xxx — 向终端/输入框发送文本（用 Input.insertText）
    // body: 要输入的文字，\n 会被转成 Enter 键
      ...
      for (const ch of text) {
        if (ch === '\n') {
          await sendCDP('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13, nativeVirtualKeyCode: 13 }, sid);
          await sendCDP('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Enter', code: 'Enter', windowsVirtualKeyCode: 13, nativeVirtualKeyCode: 13 }, sid);
        } else {
          await sendCDP('Input.dispatchKeyEvent', { type: 'keyDown', key: ch, text: ch, unmodifiedText: ch }, sid);
          await sendCDP('Input.dispatchKeyEvent', { type: 'keyUp', key: ch, text: ch, unmodifiedText: ch }, sid);
        }
      }
      res.end(JSON.stringify({ typed: text.length + ' chars' }));
    }
